...
If you're migrating from a previous version of s6-overlay (_v2_) to the
new version (_v3_), you may need to make some changes to your services
...
- Be usable on top of _any_ Docker image
...
There's nothing inherently _bad_ about running multiple processes in a container.
The more abstract "one _thing_ per container" is our policy - a container should do one thing,
...
_must_ restart failed services, meaning the Docker container will never die.
...
_source definition directory_ in the `/etc/s6-overlay/s6-rc.d` directory,
...
directory. The format of a _source definition directory_ is described in
[this page](https://skarnet.org/software/s6-rc/s6-rc-compile.html). Note that
you can define _longruns_, i.e. daemons that will get supervised by s6 just
like with the `/etc/services.d` method, but also _oneshots_, i.e. programs that
will run once and exit. Your main service is probably a _longrun_, not a
_oneshot_: you probably need a daemon to stick around.

The advantage of this new format is that it allows you to define dependencies
between services: if _B_ depends on _A_, then _A_ will start first, then _B_ will
start when _A_ is ready, and when the container is told to exit, _B_ will stop
first, then _A_. If you have a complex architecture where various processes
depends on one another, or simply where you have to mix _oneshots_ and _longruns_
...
permissions on host mounts _offline, before running the container_. This
...
start _after_ the `/etc/services.d` ones; but in order to do so, every service
...
automatically reads from the _stdout_ of your service, and logs the data
...
- myapp-log-prepare is a oneshot, preparing the logging directory.
  It is a dependency of myapp-log, so it will be started _before_ myapp-log.
...
- `S6_BEHAVIOUR_IF_STAGE2_FAILS` (default = 0): determines what the container should do
  ...
  - if any old-style `/etc/services.d` or new-style [s6-rc](https://skarnet.org/software/s6-rc/) longrun is marked
    as expecting readiness notification, and fails to become _ready_ in the allotted time (see
...
- `S6_KILL_FINISH_MAXTIME` (default = 5000): How long (in milliseconds) the system should
  ...
  milliseconds for _each_ script.
...
- `S6_KILL_GRACETIME` (default = 3000): How long (in milliseconds) `s6` should wait, at the end of
  ...
  before sending a `KILL` signal to make _sure_ they're dead.
...
- `S6_CMD_WAIT_FOR_SERVICES` (default = 0): By default when the container starts,
  ...
  will wait until the services in `/etc/services.d` are _ready_ before proceeding
...
Generally speaking, you _should not_ run your containers with `docker run -it`.
...
method of stopping them. s6-overlay _tries_ to exit cleanly on ^C, whether the
...
- `s6-overlay-${arch}.tar.xz`: the binaries for platform _${arch}_.
...
one is _worse_ than others because its naming is inconsistent. The gcc
